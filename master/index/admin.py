...
from libs.fastapi_amis_admin.amis import Form, TableCRUD
from libs.fastapi_amis_admin.amis.components import Page, PageSchema
from libs.fastapi_amis_admin.crud import BaseApiOut, ItemListSchema
from libs.fastapi_amis_admin.crud.base import SchemaCreateT
...
class NavPageAdmin(admin.ModelAdmin):
    ...
    def __init__(self, app: "AdminApp"):
        ...
        async def sync_pages():
            await self.site.db.async_run_sync(
                lambda session: AmisPageManager(session)
                .site_to_db(self.site)
                .db_to_site(self.site)
            )
    ...
    async def get_page(self, request: Request) -> Page:
        ...
        page.title = "页面管理"
        page.toolbar = amis.ActionType.Ajax(
            label="更新应用页面",
            api=f"{self.router_path}/reload",
            level=amis.LevelEnum.primary,
        )
        return page
    ...
    def register_router(self):
        ...
        async def reload_site_page_schema(request: Request):
            await self.db.async_run_sync(
                lambda session: AmisPageManager(session).db_to_site(self.site)
            )
            return BaseApiOut(msg="success")

        @self.router.get("/get_active_pages")
        async def get_active_pages(request: Request):
            items = await self.db.async_run_sync(
                lambda session: AmisPageManager(session).get_db_active_pages()
            )
            # 将根节点的子节点提取出来
            if items:
                root = items[0]
        ...
        async def update_pages(
            request: Request, data: List[dict] = Body(..., embed=True)
        ):
            await self.db.async_run_sync(
                lambda session: AmisPageManager(
                    session
                ).update_db_pages_parent_and_sort(data)
            )
            return BaseApiOut(msg="success")
