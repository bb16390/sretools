...
from master.libs.fastapi_amis_admin import amis, models
from master.libs.fastapi_amis_admin.amis import PageSchema
...
def parse_page_schema_type(obj: PageSchema) -> NavPageType:
    if obj.schema_:
        return (
            NavPageType.Iframe if obj.schema_.type == "iframe" else NavPageType.Schema
        )
    elif obj.schemaApi:
...
class BaseNavPage(SQLModel):
    ...
    type: NavPageType = Field(
        NavPageType.Custom, title="页面类型", sa_type=ChoiceType(NavPageType)
    )
    ...
    icon: str = Field(
        ...
        amis_form_item=amis.Group(
            name="icon",
            body=[
                amis.InputText(name="icon", required=True),
                amis.Tpl(
                    label="图标预览",
    ...
    desc: str = Field(
        default="", title="页面描述", max_length=400, amis_form_item="textarea"
    )
    ...
    parent_id: Optional[int] = Field(
        None, title="上级菜单", foreign_key="system_page.id"
    )
    unique_id: str = Field(
    ...
    is_locked: bool = Field(
        False, title="是否锁定", description="不锁定,则自动同步系统页面默认配置"
    )
