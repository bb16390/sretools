...
class AmisPageManager:
    ...
    def db_to_site(self, admin_group: AdminGroup):
        ...
        def append_page_to_site(
            page_: NavPage, admin_: PageSchemaAdmin = None
        ) -> Optional[PageSchemaAdmin]:
            """添加子级菜单"""
            group = admin_group
            if page_.parent_id:
                if (
                    page_.parent.unique_id == admin_group.unique_id
                ):  # 如果父级是根级,则直接添加
                    group = admin_group
            ...
            if not admin_:
                admin_ = (
                    AdminGroup(group.app)
                    if page_.is_group
                    else PageSchemaAdmin(group.app)
                )
        ...
        def update_page_to_site(page_: NavPage) -> Optional[PageSchemaAdmin]:
            ...
            if admin:  # 如果存在,则更新
                ...
                if (
                    page_.parent_id and parent.unique_id != page_.parent.unique_id
                ):  # 如果不是根级,并且父级不一致,则更新父级
                ...
                page_.is_active and page_.visible
            ):  # 如果不存在,并且是激活的,则添加到site
        ...
        for i, link in enumerate(links):
            ...
            if link.get("children"):
                self.update_db_pages_parent_and_sort(
                    link["children"],
                    parent_id=page.id if page.is_group else page.parent_id,
                )
    ...
    def get_db_active_pages(self, parent_id: int = None) -> list[dict]:
        ...
        stmt = (
            select(NavPage)
            .where(NavPage.is_active == True, NavPage.visible == True)
            .order_by(NavPage.sort.desc())
        )
        pages = self.session.scalars(stmt)
        return include_children(
            [page.as_nav_link().amis_dict() for page in pages], key="value"
        )


def include_children(
    items: list[dict], key: str = "id", parent_key: str = "parent_id"
) -> list[dict]:
    ...
    def insert_new_node(
        node: dict, new_items: list[dict], is_top: bool = False
    ) -> bool:
        #  先把列表中的子节点全部找出来.
        for item in new_items.copy():
            if node[key] == item.get(
                parent_key, None
            ):  # 找出新节点的子节点,添加到新节点的children中
                new_items.remove(item)
                node = append_child(node, item)
        for i, item in enumerate(new_items):
            if (
                node.get(parent_key, None) == item[key]
            ):  # 如果新节点的父级是当前节点,则添加到当前节点的子节点中
