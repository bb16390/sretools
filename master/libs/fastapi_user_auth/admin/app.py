...
class UserAuthApp(AdminApp, AuthRouter):
    ...
    def __init__(self, app: "AdminApp"):
        ...
        self.UserLoginFormAdmin.schema = (
            self.UserLoginFormAdmin.schema
            or create_model_by_model(
                self.auth.user_model, "UserLoginIn", include={"username", "password"}
            )
        )
        self.UserLoginFormAdmin.schema_submit_out = (
            self.UserLoginFormAdmin.schema_submit_out or self.schema_user_login_out
        )
        self.UserRegFormAdmin.schema = (
            self.UserRegFormAdmin.schema
            or create_model_by_model(
        ...
        self.UserRegFormAdmin.schema_submit_out = (
            self.UserRegFormAdmin.schema_submit_out or self.schema_user_login_out
        )
        self.UserInfoFormAdmin.user_model = self.auth.user_model
        self.UserInfoFormAdmin.schema = (
            self.UserInfoFormAdmin.schema
            or create_model_by_model(
                self.auth.user_model,
                "UserInfoForm",
        ...
        self.UserInfoFormAdmin.schema_submit_out = (
            self.UserInfoFormAdmin.schema_submit_out or self.schema_user_info
        )
        # register admin
        self.register_admin(
    ...
    def register_router(self):
        ...
        async def site_admin_actions_options(request: Request):
            # 获取当前登录用户的权限
            username = (
                await self.auth.get_current_user_identity(request)
                or SystemUserEnum.GUEST
            )
            # 获取当前用户的权限列表
            options = get_admin_action_options_by_subject(
                enforcer=self.auth.enforcer, subject="u:" + username, group=self.site
            )
