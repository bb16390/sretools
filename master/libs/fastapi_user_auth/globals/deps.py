...
CurrentUserOrNone: Optional[g.UserModel] = Annotated[
    Optional[g.UserModel], Depends(get_user_or_none)
]
...
def get_user_or_error(user: CurrentUserOrNone):
    ...
    if not user:
        raise AuthError(
            status=ErrorCode.USER_IS_NOT_LOGIN, msg=_("User is not logged in")
        )  # 用户未登录
    return user
