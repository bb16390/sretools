...
class AuthFieldModelAdmin(BaseAuthFieldModelAdmin):
    async def has_field_permission(
        ...
        action += ""
        effect = self.site.auth.enforcer.enforce(
            "u:" + subject, self.unique_id, f"page:{action}:{field}", f"page:{action}"
        )
        return effect
...
class AuthSelectModelAdmin(BaseAuthSelectModelAdmin):
    async def has_select_permission(self, request: Request, name: str) -> bool:
        ...
        effect = self.site.auth.enforcer.enforce(
            "u:" + subject, self.unique_id, f"page:select:{name}", "page:select"
        )
        return effect
