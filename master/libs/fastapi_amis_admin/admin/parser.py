...
from libs.fastapi_amis_admin.amis import AmisNode
...
from libs.fastapi_amis_admin.amis.constants import LabelEnum
from libs.fastapi_amis_admin.models import Choices
...
class AmisParser:
    ...
    def as_table_column(
        ...
        if quick_edit:
            column.quickEdit = self.as_form_item(
                modelfield, set_default=True
            ).model_dump(exclude_none=True, by_alias=True, exclude={"name", "label"})
    ...
    def update_common_attrs(
        ...
        if not is_filter:
            ...
            item.required = modelfield.field_info.is_required() and not issubclass(
                type_, bool
            )
    ...
    def _get_form_item_from_kwargs(
        ...
        outer_type = field_outer_type(modelfield)
        ...
        kwargs = self.get_field_amis_form_item_type(
            type_=modelfield.field_info.annotation,
            is_filter=is_filter,
            required=modelfield.field_info.is_required()
            and not field_allow_none(modelfield),
        )
    ...
    def _get_table_column_from_kwargs(self, modelfield: ModelField) -> TableColumn:
        ...
        if isinstance(table_column, TableColumn):
            return table_column
        kwargs = self.get_field_amis_table_column_type(modelfield.field_info.annotation)
