...
from master.libs.fastapi_amis_admin.utils.pydantic import AllowExtraModelMixin
...
class AmisAPI(BaseAmisModel):
    ...
    data: Union[str, dict, None] = (
        None  # The requested data body, supports data mapping
    )
    ...
    qsOptions: Union[str, dict, None] = (
        None  # Useful when dataType is form or form-data.
    )
    ...
    cache: Optional[int] = (
        None  # Set cache to set the cache time, the unit is milliseconds. During the set cache time,
    )
    # the same request will not be repeated, but the cached request response data will be obtained.
    requestAdaptor: Optional[str] = (
        None  # Send adapter, API configuration of amis, if you can't configure the request
    )
    ...
    replaceData: Optional[bool] = (
        None  # Whether the returned data replaces the current data, the default is false, that is:
    )
    # append, set to true to completely replace.
    adaptor: Optional[str] = (
        None  # Receive adapter, if the interface return does not meet the requirements, you can configure
    )
    ...
    trackExpression: Optional[str] = (
        None  # Configure the tracking variable expression. When automatic refresh is enabled,
    )
