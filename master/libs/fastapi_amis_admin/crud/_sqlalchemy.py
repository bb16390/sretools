...
class SqlalchemySelector(Generic[TableModelT]):
    ...
    def __init__(
        ...
        assert hasattr(self.model, "__table__"), (
    ...
    def _select_maker(self):
        if self.link_models:
            ...
            def select_maker(
                ...
                link_clause: Annotated[
                    Optional[Any], Depends(self.get_link_clause)
                ] = None,  # type: ignore
            ) -> Select:
...
class SqlalchemyCrud(
    ...
    def _create_schema_filter(self) -> Type[SchemaFilterT]:
        ...
        for modelfield in modelfields:
            ...
            if field_annotation_is_scalar(
                modelfield.field_info.annotation
            ) and issubclass(type_, (Enum, bool, str)):
                continue
    ...
    async def on_update_pre(
        ...
        **kwargs,
        ...
        data = obj.model_dump(
            exclude=self.update_exclude, exclude_unset=True, by_alias=True
        )
        data = {
            key: val
    ...
    async def on_filter_pre(
        ...
        return obj and {
            k: v
            for k, v in obj.model_dump(exclude_unset=True, by_alias=True).items()
            if v is not None
        }
        ...
        async def route(
            ...
            if paginator.showTotal:
                data.total = await self.db.async_scalar(
                    sel.with_only_columns(func.count("*"))
                )
                if data.total == 0:
    ...
    def route_create(self) -> Callable:
        async def route(
            request: Request,
            data: Annotated[
                Union[List[self.schema_create], self.schema_create], Body()
            ],  # type: ignore
